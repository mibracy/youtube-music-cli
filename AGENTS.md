...
- Favorites video IDs are also retained in `config.json` as a backup; migration to `favorites.json` only happens when `favorites.json` is missing/empty and never drops config favorites
...
- `PlayerService.stop()` must NOT clear `currentTrackId` — the TUI play/pause effect in `player.store.tsx` matches it against `state.currentTrack.videoId` to decide between `resume()` and restarting; upstream merge d8f1315 reintroduced `currentTrackId = null` in `stop()` and broke pause/resume after the 2nd track. The stale `ipcConnectPending` `.catch` in `play()` must also return early when `isStalePlay()`, or it kills the newer session's mpv and feeds the old track's retry loop
